            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  _RoleCard(title: "I'm a Creator", subtitle: "Monetize content & grow community.", icon: Iconsax.video_tick, color: const Color(0xFF7C3AED), isSelected: selectedRole == "creator", onTap: () => setState(() => selectedRole = "creator")),
                  const SizedBox(height: 20),
                  _RoleCard(title: "I'm a Fan", subtitle: "Access exclusive creator content.", icon: Iconsax.heart5, color: const Color(0xFF06B6D4), isSelected: selectedRole == "user", onTap: () => setState(() => selectedRole = "user")),
                  const Spacer(),
                  FadeInUp(
                    child: Padding(
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(color: isSelected ? color.withOpacity(0.1) : Colors.white.withOpacity(0.03), borderRadius: BorderRadius.circular(30), border: Border.all(color: isSelected ? color : Colors.white.withOpacity(0.1), width: isSelected ? 2 : 1), boxShadow: isSelected ? [BoxShadow(color: color.withOpacity(0.2), blurRadius: 20)] : []),
        child: Row(children: [Container(height: 60, width: 60, decoration: BoxDecoration(color: isSelected ? color : Colors.white.withOpacity(0.05), borderRadius: BorderRadius.circular(20)), child: Icon(icon, color: isSelected ? Colors.white : Colors.white38, size: 30)), const SizedBox(width: 20), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)), Text(subtitle, style: GoogleFonts.outfit(fontSize: 14, color: Colors.white54))]))]),
      ),
    );
  }
}
